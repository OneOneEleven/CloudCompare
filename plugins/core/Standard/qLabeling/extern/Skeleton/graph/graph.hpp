...
#include "graph/readGraphOBJ.hpp"
#include "graph/writeGraphOBJ.hpp"
...
namespace qSkeletonLib{
	...
	public:
		...
		inline Graph(std::string file_name, Options opts);
		inline Graph(Eigen::MatrixXd nodes, Eigen::MatrixXi edges);
		inline Graph(Eigen::MatrixXd nodes, Eigen::MatrixXi edges, Options opts);
	...
	inline Graph::Graph(std::string file_name, Options opts) : Graph(file_name) 
	{
		opts_ = opts;
	};
	...
	// overload with options
	inline Graph::Graph(Eigen::MatrixXd nodes, Eigen::MatrixXi edges, Options opts) : Graph(nodes, edges) 
	{
		opts_ = opts;
	};

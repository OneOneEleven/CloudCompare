...
namespace qSkeletonLib {
    ...
    public:
        ...
        OneRing(int vertex_id, const std::vector <int>& neighbours_id, const Eigen::MatrixXd& cloud)
            : vertex_id_(vertex_id), neighbours_id_(neighbours_id)
        {
        ...
        const std::vector< int >& get_one_ring()const {
            return one_ring_;
        };
        const std::vector< int >& get_connected_components(int i)const {
